...
#'
#' @description
#' `import_tide()` is the helper function to import tandem ms search results
...
import_tide <- function(
  ...
  out <- path |>
    ...
    dplyr::rename(
      psm_mh = `peptide mass`,
      psm_score = `xcorr score`,
      ms_event = `scan`,
      psm_peptide = `sequence`,
      psm_protein = `protein id`
    ) |>
    dplyr::group_by(ms_event) |>
    dplyr::mutate(psm_rank = dplyr::row_number(),
                  ms_event = ms_event + 1) |>
    dplyr::ungroup() |>
    dplyr::select(-c('target/decoy'))
